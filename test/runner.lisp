...
(defun system-path ()
  (asdf:system-source-directory "cl-braces.language/tests"))
...
(defun run-vm-suites ()
  (let ((snapshots:*snapshot-dir* (merge-pathnames "test/snapshots/vm/" (system-path))))
...
(defun run-compiler-suites ()
  (let ((snapshots:*snapshot-dir* (merge-pathnames "test/snapshots/compiler/" (system-path))))
    (with-env-specific-setup
      (run-tests
       :name "compiler"
