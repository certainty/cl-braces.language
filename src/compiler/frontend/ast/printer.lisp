...
(defclass ast-printer ()
  ((indentation-level
   ...
   (type-map
    :initarg :type-map
    :initform nil
    :documentation "A hash table that maps types to their respective printers.")))

(defun print-ast (ast &key (stream *standard-output*) (print-spans-p nil) (type-map nil))
  "Prints the give `AST' to  `STREAM' in a human readable format."
  (let ((printer (make-instance 'ast-printer :stream stream :print-spans-p print-spans-p :type-map type-map)))
    (walk printer ast)))
...
(defun node-type (printer node)
  (with-slots (type-map) printer
    (if (null type-map)
        ""
        (a:if-let ((tpe (types:fetch-type type-map node)))
          (format nil " :~A" (types:type-name tpe))
          ""))))

(defun print-node (printer node caption &key (leafp nil))
  (with-slots (indentation-level stream print-spans-p) printer
    (format stream "~A~A~A" (connective indentation-level) caption (node-type printer node))
    (when print-spans-p
...
(define-default-printer return-statement "return-statement")
