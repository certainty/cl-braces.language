...
(define-condition scan-error (error)
  ((message
    ...
    :initarg :message )
   (span
    :reader scan-error-span
    :initarg :location))
  (:report (lambda (condition stream)
             (format stream "Illegal token at ~a" (scan-error-span condition)))))
...
(defclass state ()
  ((source-code
   ...
   (token-ids
    :initarg :token-ids
    :initform (error "no token ids given"))

   (span-map
    :initarg :span-map
    :initform (error "no span map given")
    :documentation "A map that keeps track of the spans of the scanned tokens.")))

(defmethod print-object ((state state) stream)
  (print-unreadable-object (state stream :type t)
    (format stream "last-token: ~a cursor-base: ~a cursor-advanced: ~a location: ~a"
            (slot-value state 'last-token)
            (slot-value state 'cursor-base)
            (slot-value state 'cursor-advanced)
            (slot-value state 'location-base))))

;;; ================================
;;; Main API
;;; ================================

(-> make-scanner (string support:id-generator span:span-map &key (:fail-fast boolean)) state)
(defun make-scanner (source-code token-ids span-map &key (fail-fast nil))
  ...
  (prog1 (make-instance 'state :source-code source-code :fail-fast fail-fast :token-ids token-ids :span-map span-map)))

(defun all-tokens (scanner)
  "Scans the given `source-code' and returns a list of all the tokens that were scanned. "
  (let ((result  (make-array 0 :adjustable t :fill-pointer 0 :element-type 'token:token)))
...
(a:define-constant +whitespace+ (list #\Space #\Tab #\Return #\Newline) :test #'equal)
...
(defun scan-inter-token-space (state)
  (with-slots (cursor-base cursor-advanced location-base) state
    ...
    (when (peek= state "//")
      (advance state)
      (advance state)
      (scan-until state #'(lambda (c) (equal c #\Newline)))
      (scan-while state #'inter-token-space-p)
      (skip state))))
...
(defun accept (state token-class &key (coerce-with #'identity) (synthetic nil))
  ...
  (with-slots (cursor-base cursor-advanced location-base source-code last-token had-errors span-map token-ids) state
    (let* ((lexeme (subseq source-code cursor-base cursor-advanced))
           (span (update-locations state)))
      (setf cursor-base cursor-advanced)
      (let ((token-id (support:next-id token-ids)))
        (setf last-token (make-instance 'token:token
                                        :class token-class
                                        :lexeme (if synthetic "synth" lexeme)
                                        :id token-id
                                        :value
                                        (funcall coerce-with lexeme))))
      (prog1 last-token
        (setf (span:fetch-span span-map last-token) span)
        (when (token:class= last-token token:@ILLEGAL)
          (setf had-errors t)
          (cerror "Illegal token" (make-condition 'scan-error :message "Illegal token" :span span)))))))
