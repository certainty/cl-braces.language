...
(defclass state ()
  ((scanner
   ...
   (node-ids
    :initarg :node-ids
    :initform (error "no node ids given"))
   (span-map
    :initarg :span-map
    :initform (error "no span map given")
    :documentation "A map that keeps track of the spans of the parsed input."))
...
(-> make-parser ((vector token:token *) support:id-generator span:span-map &key (:fail-fast boolean)) state)
(defun make-parser (tokens node-ids span-map &key (fail-fast nil))
  "Create a new parser instance with the given `TOKENS' and `NODE-IDS'."
  (prog1 (make-instance 'state :scanner (make-parse-buffer tokens) :node-ids node-ids :span-map span-map :fail-fast fail-fast)))

;;; ====================================================================================================
;;; Main API for the parser
;;; ====================================================================================================

(-> parse (state &key (:production function) (:expect-full-parse boolean)) (values (or null ast:node list) boolean state &optional))
(defun parse (state &key (production #'<source-file) (expect-full-parse t))
  "Parse the given `TOKENS' and return values (AST, ERRORS, STATE).
  ...
  (with-slots (fail-fast) state
    (handler-bind ((error-detail (lambda (c)
      ...
      (let ((result (funcall production state)))
        ;; when we have parse errors, there is no point in expecting a full parse
        (when (and expect-full-parse (null (parse-errors state)))
...
(defun <source-file (state)
  ...
  (let ((package (expect #'<package-declaration "Expected package clause" state))
                                        ;(imports (try #'<import-declaration state))
...
(defun <if-statement (state)
  (guard-parse state
    (with-slots (cur-token node-ids) state
      (when (token:class= cur-token token:@IF)
        ...
        (let* ((init (<simple-statement state))
          ...
          (cond
            ...
            (t
             (setf condition init)
             (setf init (make-instance 'ast:empty-statement :id (support:next-id node-ids)))))
...
(defun accept (state node-class &rest args)
  (with-slots (cur-token node-ids) state
    (if cur-token
        (apply #'make-instance node-class :id (support:next-id node-ids) args)
