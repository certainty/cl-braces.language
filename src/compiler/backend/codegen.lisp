...
(defclass bytecode-generator ()
  ((current-package-name
   ...
   (label-ids
    :initform (support:make-id-generator :prefix  "")
    :type support:id-generator
    :documentation "The next label id to use")
   (block-labels
    :initform (make-hash-table :test #'equal)
    :type hash-table
...
(-> make-generator (symbols:symbol-table) bytecode-generator)
(defun make-generator (symbol-table)
  (prog1 (make-instance 'bytecode-generator :symbol-table symbol-table)))
...
(defun create-label (generator prefix)
  (with-slots (label-ids block-labels instructions) generator
